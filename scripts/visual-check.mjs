...
const position = await call('Runtime.evaluate', { expression: '({ scrollY, active: document.querySelector("header nav a.active")?.textContent, hash: location.hash })', returnByValue: true })
console.log(JSON.stringify({ output, overflow: overflow.result.value, position: position.result.value, issues }, null, 2))
socket.close()
